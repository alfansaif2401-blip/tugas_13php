<?php
// Script to create the SISTAMIK database and tables
...
// Create database
$sql = "CREATE DATABASE IF NOT EXISTS db_sistamik";
if (mysqli_query($conn, $sql)) {
    echo "Database 'db_sistamik' created successfully\n";
} else {
    echo "Error creating database: " . mysqli_error($conn) . "\n";
}

// Select database
mysqli_select_db($conn, 'db_sistamik');
...
if (mysqli_query($conn, $sql)) {
    ...
    echo "Error creating jurusan table: " . mysqli_error($conn) . "\n";
}
...
if (mysqli_query($conn, $sql)) {
    echo "Mahasiswa table created successfully\n";
} else {
    echo "Error creating mahasiswa table: " . mysqli_error($conn) . "\n";
}

// Insert sample jurusan
$sql = "INSERT INTO jurusan (nama_jurusan, kode_jurusan) VALUES
...
if (mysqli_query($conn, $sql)) {
    echo "Sample jurusan inserted successfully\n";
} else {
    echo "Error inserting jurusan: " . mysqli_error($conn) . "\n";
}

// Insert sample mahasiswa
$sql = "INSERT INTO mahasiswa (nim, nama_lengkap, jenis_kelamin, tempat_lahir, tanggal_lahir, alamat, no_telepon, email, id_jurusan, angkatan, status, tanggal_masuk) VALUES
...
if (mysqli_query($conn, $sql)) {
    echo "Sample mahasiswa inserted successfully\n";
} else {
    echo "Error inserting mahasiswa: " . mysqli_error($conn) . "\n";
}

mysqli_close($conn);
echo "SISTAMIK database setup completed!\n";
?>
